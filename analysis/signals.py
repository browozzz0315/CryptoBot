    ...
    if macd_cross == "bullish":
        events.append(
            SubscriptionEvent(
                ...
                summary="最新一根 K 線顯示 MACD 由下往上穿越 Signal。",
            )
        )
        ...
        events.append(
            SubscriptionEvent(
                ...
                summary="最新一根 K 線顯示 MACD 由上往下跌破 Signal。",
            )
        )
    ...
    if oi_change_pct is not None and oi_change_pct >= oi_surge_threshold_pct and abs(change_24h) <= price_flat_threshold_pct:
        events.append(
            SubscriptionEvent(
                ...
                title=f"{symbol} OI 暗流累積",
                summary=f"OI 變化 {oi_change_pct:+.2f}%，但 24h 價格僅 {change_24h:+.2f}%，有倉位累積跡象。",
                value=oi_change_pct,
            )
        )
